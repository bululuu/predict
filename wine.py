...
# 读取文件
for info in os.listdir(‘C:\Users\bululu\Documents\Untitled Folder\wine\benchmarks’): 
domain = os.path.abspath(r’C:\Users\bululu\Documents\Untitled Folder\wine\benchmarks’)  
info = os.path.join(domain,info)  
df = pd.read_csv(info) 
#规格化
from sklearn.preprocessing import MinMaxScaler
scaler = MinMaxScaler(feature_range=(0, 1))
df[['diff.score','alcohol']] = scaler.fit_transform(df[['diff.score','alcohol']])
df[['diff.score','alcohol']].head()
X1 = df['diff.score'].values.reshape(-1,1)
X2 = df['alcohol'].values.reshape(-1,1)
...
for i, (clf_name, clf) in enumerate(classifiers.items()):
    ...
    IX2 =  np.array(dfx['alcohol'][dfx['outlier'] == 0]).reshape(-1,1)
    
    # OX1 - outlier feature 1, OX2 - outlier feature 2
    OX1 =  dfx['diff.score'][dfx['outlier'] == 1].values.reshape(-1,1)
    OX2 =  dfx['alcohol'][dfx['outlier'] == 1].values.reshape(-1,1)
